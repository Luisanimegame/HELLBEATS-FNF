...
class PlayState extends MusicBeatState
{
	...
	var bg:FlxSprite; // have to make these dumb variables outside of the switch for modcharts
	...
	var cityDark:FlxSprite;
	var streetDark:FlxSprite;
	...
	override public function create()
	{
		...
		//dialogue shit
		if(Assets.exists(Paths.txt(songLowercase + '/' + songLowercase + 'Dialogue')))
		...
		switch(stageCheck)
		{
			...
			case 'philly': 
					...
					streetBehind = new FlxSprite(-40, 50).loadGraphic(Paths.image('philly/behindTrain','week3')); //MAKING THIS SO THAT STREETDARK WORK
					streetBehind.visible = false;
					add(streetBehind);
					
					bgLight = new FlxSprite(-100).loadGraphic(Paths.image('philly/skydrop', 'week3'));
					bgLight.scrollFactor.set(0.1, 0.1);
					bgLight.alpha = 1;
					bgLight.visible = false;
					add(bgLight);

					cityDark = new FlxSprite(-10).loadGraphic(Paths.image('philly/citydrop', 'week3'));
					cityDark.scrollFactor.set(0.3, 0.3);
					cityDark.setGraphicSize(Std.int(cityDark.width * 0.85));
					cityDark.updateHitbox();
					cityDark.alpha = 1;
					cityDark.visible = false;
					add(cityDark);

					streetDark = new FlxSprite(-40, streetBehind.y).loadGraphic(Paths.image('philly/streetdrop','week3'));
					streetDark.alpha = 1;
					streetDark.visible = false;
					add(streetDark);
		...
		healthBar.createFilledBar(0xFFFF0000, 0xFFf4ff00);
		// healthBar
		add(healthBar);
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong," ", "-").toLowerCase())
			{
				...
				default:
					...
					if(Assets.exists('assets/data/' + songLowercase + '/' + songLowercase + 'Dialogue.txt'))
						schoolIntro(doof);
					else
						startCountdown();
			}
		...
		else
		{
			switch (curSong.toLowerCase())
			{
				default:
					...
					if(Assets.exists('assets/data/' + songLowercase + '/' + songLowercase + 'Ending.txt'))
						schoolIntro(doof);
					else
						startCountdown();
			}
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		switch(songLowercase)
		{
			...
			default:
				// do less when normal cuz pixel is special
				var black:FlxSprite = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
				black.scrollFactor.set();
				add(black);
				new FlxTimer().start(0.3, function(tmr:FlxTimer)
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				case 'normal':
					babyArrow.frames = Paths.getNoteAssets(storyWeek);
	...
	override function closeSubState()
	{
		if (paused)
		{
			if (FlxG.sound.music != null && !startingSong && !inCutscene)
			{
				resyncVocals();
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if(SONG.song.toLowerCase() == 'blammed'){
			if(curStep == 640){
				...
				bg.visible = false;
				cityDark.visible = true;
				streetDark.visible = true;
		...
		if (startingSong && (!songEnded || !inCutscene))
		{
			if (startedCountdown)
			{
						...
						if (storyPlaylist.length <= 0)
							{
								...
								if(!Assets.exists('assets/data/' + songLowercase + '/' + songLowercase + 'Ending.txt')){
									...
									FlxG.switchState(new EndingDialogue());
								}
								#if windows
								if (luaModchart != null)
								{
	...
	*/
	...
	{
		super.stepHit();
		if (FlxG.sound.music.time > Conductor.songPosition + 20 || FlxG.sound.music.time < Conductor.songPosition - 20 && !inCutscene)
		{
			resyncVocals();
		}
